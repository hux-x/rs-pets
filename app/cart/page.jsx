...
const CartHeroBanner = ({ itemCount }) => (
  <div className="relative bg-gradient-to-br from-[#0f2d5e] via-[#1a4a8a] to-[#0e7fc4] pt-10 pb-24 px-4 sm:px-6 lg:px-12 overflow-hidden">
    ...
    <div className="absolute bottom-0 left-0 right-0 pointer-events-none">
      <svg viewBox="0 0 1440 80" preserveAspectRatio="none" className="w-full h-14 sm:h-20 fill-gray-50">
...
const EmptyCart = ({ goToPage }) => (
  <div className="min-h-screen bg-gray-50">
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 -mt-10 relative z-10 pb-24">
      <motion.div
        className="bg-white rounded-3xl shadow-[0_16px_48px_rgba(14,127,196,.1)] border border-blue-50 flex flex-col items-center justify-center py-20 px-6 text-center"
      ...
      >
        ...
        >
          <ShoppingBag size={52} className="text-blue-100 mx-auto mb-5" />
        </motion.div>
        ...
        <motion.button
          ...
          className="flex items-center gap-2 px-7 py-3 bg-gradient-to-br from-[#1a4a8a] to-[#0e7fc4] text-white font-bold rounded-2xl shadow-[0_6px_24px_rgba(14,127,196,.35)] text-sm"
...
const QuantityStepper = ({ quantity, onDecrease, onIncrease, disabled }) => (
  <div className="inline-flex items-center rounded-xl border-2 border-blue-100 overflow-hidden bg-white shadow-sm">
    <motion.button
      onClick={onDecrease}
      className="w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center text-blue-600 hover:bg-blue-50 transition-colors"
      whileTap={{ scale: 0.85 }}
      aria-label="Decrease"
    >
      <Minus size={12} />
    ...
    <AnimatePresence mode="popLayout">
      <motion.span
        key={quantity}
        className="w-8 sm:w-10 text-center text-sm font-bold text-gray-800 tabular-nums"
    ...
    <motion.button
      onClick={onIncrease}
      disabled={disabled}
      className="w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center text-blue-600 hover:bg-blue-50 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
...
const CartRow = ({ item, productdata, currency, onQuantityChange, onRemove, index }) => {
  ...
  const isOutOfStock =
    !productdata.inStock || (productdata.stock && item.quantity > productdata.stock);
  ...
  return (
    <motion.div
      ...
      className={`bg-white rounded-2xl border border-blue-50 shadow-sm hover:shadow-[0_8px_32px_rgba(14,127,196,.1)] transition-shadow duration-300 p-3 sm:p-5 ${
    ...
    >
      ...
      <div className="flex gap-3 sm:gap-4">
        {/* Image */}
        <motion.div
          className="relative w-16 h-16 sm:w-24 sm:h-24 rounded-xl overflow-hidden bg-gradient-to-br from-blue-50 to-sky-50 flex-shrink-0 border border-blue-100"
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-between">
          ...
          <div className="flex items-start justify-between gap-2">
            ...
            <motion.button
              ...
              className="flex-shrink-0 w-7 h-7 sm:w-8 sm:h-8 rounded-xl flex items-center justify-center text-gray-300 hover:text-red-400 hover:bg-red-50 transition-colors"
          ...
          <div className="flex items-center justify-between mt-2 sm:mt-3 flex-wrap gap-2">
            ...
            <div className="text-right">
              <AnimatePresence mode="popLayout">
                <motion.p
                  key={itemTotal}
                  className="font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-[#1a4a8a] to-[#0e7fc4] text-sm sm:text-base"
...
const MissingProductRow = ({ item, onRemove }) => (
  <motion.div
    ...
    animate={{ opacity: 1 }}
    exit={{ opacity: 0 }}
    className="bg-red-50 border border-red-100 rounded-2xl p-4 flex items-center justify-between gap-3"
  >
    <p className="text-red-500 text-sm font-medium">Product no longer available</p>
    <button
      onClick={() => onRemove(item.productId, item.size)}
      className="text-red-400 text-xs underline hover:text-red-600 transition-colors whitespace-nowrap"
    >
      Remove
    </button>
  </motion.div>
...
const MobileStickyBar = ({ cartProducts, cartitems, currency, deliveryFee, onCheckout }) => {
  ...
  return (
    <motion.div
      className="fixed bottom-0 left-0 right-0 z-50 lg:hidden bg-white border-t border-blue-100 shadow-[0_-8px_32px_rgba(14,127,196,.12)] px-4 py-3"
      initial={{ y: 80 }}
      animate={{ y: 0 }}
    ...
    >
      <div className="flex items-center justify-between gap-4 max-w-xl mx-auto">
        <div>
          ...
          <p className="font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-[#1a4a8a] to-[#0e7fc4] text-base">
...
const Cart = () => {
  ...
  if (error) return (
    <div className="min-h-screen bg-gradient-to-br from-[#0f2d5e] via-[#1a4a8a] to-[#0e7fc4] flex flex-col items-center justify-center gap-4 px-4">
      <PawPrint className="w-14 h-14 text-white/30" />
      <p className="text-white/70 text-base font-medium text-center">{error}</p>
      <motion.button
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 -mt-10 relative z-10 pb-32 lg:pb-24">
        <div className="grid lg:grid-cols-[1fr_360px] gap-6 lg:gap-8 items-start">
          ...
          <div className="space-y-3 sm:space-y-4">
            ...
            >
              <button
                onClick={() => goToPage("/shop")}
                className="flex items-center gap-2 text-sm text-blue-500 hover:text-blue-700 font-medium transition-colors mt-1"
              >
                <ArrowLeft size={15} />
                Continue Shopping
              </button>
